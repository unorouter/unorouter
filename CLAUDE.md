...
- Two DBs, both Drizzle ORM / SQLite dialect:
  ...
  - Client: SQLocal over OPFS, lazy WASM import (~1.5MB). `src/lib/db/client/client.ts` `getLocalDb(userId)`, per-user OPFS file `${appName}-${userId}.sqlite3`. Per-userId promise cache. Releases SyncAccessHandle on `pagehide`/`beforeunload`. Salvage (prod too): failed migration opens a fresh DB at a temp path, copies surviving rows via `copyAllTables`, overwrites the original file; copy failure falls back to wipe. SELF-HEAL: every statement (drizzle driver + exec + transaction) runs through a guard in `buildLocalClient`; on GetSyncHandleError/NotFoundError/SQLITE_CORRUPT/SQLITE_IOERR (site data cleared while the app is open kills the SyncAccessHandle) it single-flight reopens via the salvage-capable `openMigratedSql` and replays the failed statement, so persistence recovers without a reload.
