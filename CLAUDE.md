...
- `route.ts`: `GET /:id/meta`, `GET /:id/markdown`, `POST /title`, `POST /stream` (answers 409 `{code:"context-required"}` on a context-cache miss; guest streams force `webSearch=false`), `GET /task/:taskId`, `POST /:id/task/finalize`. API keys resolve via `resolveChatApiKey` (`billing/token/best-key.service.ts`: client-store cookie -> best-key upstream lookup -> guest key). No POST/PATCH/DELETE for conversation or messages: those live in client hooks (`src/hooks/ai/chat-hook.ts`) writing the local SQLocal DB.
- `conversation.service.ts`: `getConversation` + `getConversationMarkdown` (Turso reads; markdown walks the active branch).
- `model-flags.ts`: per-model role-handling flags (RisuAI LLMFlags port) keyed off the model NAME (GLM/DeepSeek/Kimi/Gemini/Claude/GPT families). Auto-flags OR with the preset's manual flags; a manual flag is never silently turned off.
...
- `macros.ts`: CBS macro engine, full RisuAI port (field tokens, seeded roll/random/pick stable across regenerates, calc RPN, string/array/dict ops, conversation/global/temp vars, history readers, `{{#if}}`/`{{#when}}` blocks; app-coupled macros resolve to safe empties). Var writes mutate the scope in place and ride back to the client via finish-meta writebacks (`vars`, `globalVars`).
...
- `run-triggers.ts` + `example-messages.ts`: server-side `start`-mode trigger execution (system-prompt injection, var mutations, stop flag) and exampleMessages parsing into role-tagged few-shot turns.
...
- `chat-runtime-provider.tsx`: wraps tree with `AssistantRuntimeProvider`, drives a `useRemoteThreadListRuntime` and inner `useAISDKRuntime(useChat<ChatUIMessage>, { adapters: {attachments, history} })`. The transport is a `DefaultChatTransport` whose `body: async () => ({ model, convId, webSearch, overrides, chatContext, chatContextHash, globalVars, speakingCharacterId })` reads live atoms via `chatStore.get` and builds `chatContext` from SQLocal via a DYNAMIC import of `buildChatContextFromLocalDb` (keeps ~110KB of lorebook/trigger machinery out of first-paint chunks). Context-dedup handshake: full `chatContext` only when its FNV-1a fingerprint changed since the last send for this conv, else hash only; the transport's `fetch` wrapper retries ONCE with the full payload on a 409 `context-required`. `prepareSendMessagesRequest` trims the uploaded history window when memory is off (rolling-summary convs need absolute indices, send full). `useChatHelpersBridge` publishes `setMessages/getMessages/sendEmpty` via `chatHelpersAtom` so edit/delete/empty-send in `thread.tsx` can drive `useChat` from outside the React tree. `useConvIdSync` mirrors the active thread's `remoteId` into `convIdAtom`. `useModelSync` is two-way: the server conv's model seeds `chatModelAtom` on thread load; later atom changes write `upsertLocalConversationSettings` + `mirrorConvRowIfSynced`. The wrapped `sendMessage` also: acquires a per-conv cross-tab stream lock (released in onFinish/onError); runs multi-character rotation (`computeSpeakingOrder` via `groupOrder`, one VISIBLE sequential assistant stream per speaker, `speakingCharacterIdAtom` tags each); and `onFinish` may auto-continue (conv opt-in `autoContinue`, reply not ending in terminal punctuation, max 3 chained argless sends, RisuAI parity).
...
- `thread-list-adapter.ts`: pure local-first `RemoteThreadListAdapter`. `initialize` seeds the new conv row from `chatDefaultsAtom` + binds the sticky `chatLoadoutAtom` loadout (preset/persona/characters/lorebooks auto-equipped on every new chat). `rename`/`delete` use `mirrorConvRowIfSynced` / `unmirrorIfSynced` (outbox rows). `generateTitle` calls `POST /chat/title`, persists locally, mirrors patch if synced.
...
`src/lib/db/schema/client.ts`: client-only — `local_pending_sync` (PK `(kind, id)`; outbox with `hint`/`msg_ids`/`seq` columns; `MAX_PENDING_ATTEMPTS=5` from `src/lib/db/client/sync/pending-sync.ts`, `SyncKindName` + `PendingSyncOp` narrowing) and `local_meta` KV (holds `migration_version` cursor). `LOCAL_ONLY_TABLES` excludes both from cross-DB copy/salvage.
...
Client hydrator (`src/lib/db/client/sync/sync-state-hydrator.ts`): React component mounted in chat + playground layouts. Per-userId fire-once. Stage 0 (logged-in only): `migrateGuestLocalDb(userId)` folds guest rows into the user DB (idempotent single-flight in `data-migrate/guest-migrate.ts`; the hydrator is the ONLY call site, so stage 1 always reads post-copy state and no query invalidation is needed). Stage 1 (always): serial reads from local SQLocal seed the React Query cache (raw arrays for RP, page-shaped for convs `useInfiniteQuery`, `{items}` for playground sessions). Stage 2 (logged-in only): `GET /sync/state` returns `{ kind: [{id, updatedAt, syncExpiresAt}] }`; compare per-row `updatedAt` (local-newer rows are SKIPPED + logged), pull newer bundles via the batched `POST /bundles` (per-row `GET .../bundle` fallback on deploy skew), `applyBundle` upserts locally. The reconcile runs in `requestIdleCallback`; conv-page loads skip the active conv's bundle pull (SSR already covered it). Stage 3: `drainPending(userId)`. Serial throughout (sqlocal transactionMutex). `rehydrateMedia` enforces the asymmetric base64 rule.

Outbox (`src/lib/db/client/sync/pending-sync.ts`): the ONLY push path. `enqueueSync(userId, kind, id, op, {hint, msgIds})` upserts a `local_pending_sync` row by `(kind, id)`, unioning conversation hints (`full`/`row`/`bindings`/`msgs`; `full` absorbs the rest) and msgIds, resetting backoff, bumping `seq`. `drainPending(userId)` rebuilds payload(s) per row via `buildPendingPushes` (partial conv hints expand to an upsert push + a replace bindings push; everything else is `buildSyncPayload` full rebuild), POSTs with `keepExpiry: true`, evicts media base64 after full conv/playground pushes, then deletes the row ONLY if `seq` is unchanged (an enqueue that landed mid-push survives for the next drain). Failure bumps attempts (max `MAX_PENDING_ATTEMPTS=5`) + records `last_error`. `drainSoon(userId)` is the 250ms-debounced post-enqueue drain; `safeDrain` wraps both it and the scheduler tick in the cross-tab lock. No payload snapshots exist: coalescing N rapid edits into one row stays correct because drain reads current local state.

Client sync infrastructure (`src/lib/db/client/sync/`): `sync-state-hydrator.ts`, `pending-sync.ts`, `build-payload.ts` (cascade payload per kind), `evict-media.ts` (drops base64 after successful R2 upload by server), `resource-lock.ts` (cross-tab single-holder lock, `LOCK_TTL_MS` expiry covers crashed holder), `scheduler.ts` (periodic retry drain on focus/online; happy path is `drainSoon` after each enqueue).
