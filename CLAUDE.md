...
- Atoms load their cookie AFTER mount (no `getOnInit`). Two exceptions, each `getOnInit: true` PAIRED with a server-seeding provider; removing either half breaks them. `chatStoreAtom` + `ChatStoreProvider`: brings back the #418 mismatch or the cookie-spread data loss. `userThemeAtom` (`src/lib/theme/theme-store.ts`) + `UserThemeStoreProvider`: the layout bakes the theme into `<style id="user-theme">` and `UserThemeProvider`'s effect rewrites that tag from the atom, so an unseeded first render overwrites it with default CSS and every chat markdown colour disappears until the user edits the theme by hand.
- Theme: every knob is a token in `src/lib/theme/tokens.ts` or a preset feeding `generatePalette`; CSS variables are emitted only by `build-css.ts`, and static readers of a token var live in `globals.css`. Scopes (`chat`, `image`) carry only their own overrides; inheritance is the cascade. The cookie shape is versioned (`v: 2`) and `migrateTheme` runs on every read, so old cookies, history rows and files keep working.
...
- Stream transform order in `pipeline/stages/role-transform.ts` is LOCKED and has no marker: `dropFailedAssistantTurns`, `stripReasoningParts`, `stripSystemRole`, `demoteLateSystem`, `dropEmptyMessages`, `appendPrefill`, `mergeAlternateRoles`, `prependUserStub`, `appendUserStub` (skipped only when a prefill is the intended trailing assistant AND the adapter sets `prefillSupported`; Gemini 400s on a trailing model turn, so it still closes with the stub). Each earlier step exists so a later one can fold or cannot recreate what it removed.
