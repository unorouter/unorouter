...
- Server-side caches: pricing 5min (`pricing-cache.ts`, internal stream-service consumers), badge stats + pricing 5min (`badge/lib/cache.ts`), web-bot-auth directory 10min (`verify-inbound.ts`). PUBLIC upstream GETs opt into the Next Data Cache for 1h via `PUBLIC_CACHE` (`{ next: { revalidate: 3600 } }` in `src/lib/config/constants.ts`, spread into the Orval call options; the customFetch mutator passes it to fetch): `models/pricing`, `pricing/subscriptions`, `rankings`, `perf-metrics`, `ops/stats/history`, plus the GUEST branch of `billing/core/topup-info` + `subscription-plans`. RULE: every `PUBLIC_CACHE` call must be user-independent; pair it with `ADMIN_HEADERS` (explicit Authorization makes customFetch skip the per-user cookie attach) because the Data Cache keys by URL only and a cookie-flavored response would be served to everyone. Logged-in branches pass through uncached (Next 16 fetch default = no-store). `model-status` stays uncached (live data).
