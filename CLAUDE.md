...
- Two DBs, both Drizzle ORM / SQLite dialect:
  ...
  - Client: SQLocal over OPFS, lazy WASM import (~1.5MB). `src/lib/db/client/client.ts` `getLocalDb(userId)`, per-user OPFS file `${appName}-${userId}.sqlite3`. Per-userId promise cache. Releases SyncAccessHandle on `pagehide`/`beforeunload`. Salvage (prod too): failed migration opens a fresh DB at a temp path, copies surviving rows via `copyAllTables`, overwrites the original file; copy failure falls back to wipe. Connection lifecycle lives in `src/lib/db/client/connection.ts`: `openMigratedSql` (open + migrate + salvage) and `LocalDbConnection` whose `run()` SELF-HEALS - on GetSyncHandleError/NotFoundError/SQLITE_CORRUPT/SQLITE_IOERR (site data cleared while the app is open kills the SyncAccessHandle) it single-flight reopens via `openMigratedSql` and replays the failed statement, so persistence recovers without a reload. `client.ts` is just the per-user cache + LocalClient wiring; every statement path (drizzle driver, batch, exec, transaction) routes through `conn.run`.
...
- `thread-list-adapter.ts`: pure local-first `RemoteThreadListAdapter`. `initialize` seeds new conv + settings row from `chatDefaultsAtom`. `rename`/`delete` use `mirrorConvRowIfSynced` / `unmirrorIfSynced` (outbox rows). `generateTitle` calls `POST /chat/title`, persists locally, mirrors patch if synced.
...
- Server-side caches: pricing 5min (`pricing-cache.ts`, internal stream-service consumers), badge stats + pricing 5min (`badge/lib/cache.ts`), web-bot-auth directory 10min (`verify-inbound.ts`), stats history summary 5min (`ops/stats/route.ts`; upstream dump exceeds the 2MB Next data-cache item limit). PUBLIC upstream GETs opt into the Next Data Cache for 1h via `PUBLIC_CACHE` (`{ next: { revalidate: 3600 } }` in `src/lib/config/constants.ts`, spread into the Orval call options; the customFetch mutator passes it to fetch): `models/pricing`, `pricing/subscriptions`, `rankings`, `perf-metrics`, `ops/stats/history`, plus the GUEST branch of `billing/core/topup-info` + `subscription-plans`. RULE: every `PUBLIC_CACHE` call must be user-independent; pair it with `ADMIN_HEADERS` (explicit Authorization makes customFetch skip the per-user cookie attach) because the Data Cache keys by URL only and a cookie-flavored response would be served to everyone. Logged-in branches pass through uncached (Next 16 fetch default = no-store). `model-status` stays uncached (live data).
