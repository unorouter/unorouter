...
- BFF entrypoint (`src/app/api/[[...route]]/route.ts`, Elysia): mounts `webBotAuthPlugin` + 5 domain routes + openapi plugin. No requestId/timing/error middleware at root; `x-request-id` is forwarded by `deriveUpstream` when the caller supplies it. Openapi root carries MPP `x-service-info` annotations.
...
- COEP isolation: chat + playground layouts get COEP `require-corp` / COOP `same-origin`. `_next/static/*` and `/api/*` get CORP `same-origin` via both `next.config.ts` headers and `proxy.ts` runtime stamping (dev/turbopack bypasses `headers()`).
...
The only vertical with real business logic, under `src/server/ai/chat/`. Organized into core plus two subfolders:
...
Pending queue (`src/lib/db/client/sync/pending-sync.ts`): `enqueuePending(userId, kind, id, op, err?)` upserts a `local_pending_sync` row by `(kind, id)`. `drainPending(userId, payloadFor?)` replays up to `MAX_ATTEMPTS=5`; on success deletes the row, on failure bumps attempts + records `last_error`. Payload is rebuilt from current local state via `buildSyncPayload(userId, kind, id)` (theme is special-cased).
