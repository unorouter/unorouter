...
import { PUBLIC_CACHE } from "@/lib/config/constants";
    ...
    // MPP x-payment-info per paymentauth.org draft. intent=session (checkout URLs); amount null (user-picked/plan-dependent). Cast: OperationObject lacks x-* index.
const xPaymentInfo = (
...
export const billingRoute = new Elysia({ prefix: "/core" })
  ...
  .get("/topup-info", async ({ upstream }) => {
        // Guest branch cacheable; logged-in uncached (Data Cache keys by URL only, a cached cookie-authed response would leak across users).
