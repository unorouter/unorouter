// Inlay image generation (Risu runImgGen / Lua generateImage): one image via
// the first image-capable catalog model, returned as base64 for client-side
// media persistence ({{inlay::<mediaId>}} tokens render from the local media
// table). Sync endpoints carry no negative-prompt field; it is ignored.
