    // RisuAI-style prompt template: user-orderable typed cards walked into the final prompt. Cards are named SLOTS (pre-built content) or LITERAL text blocks, plus one chat card marking where history splices in.

export type PromptItemRole = "system" | "user" | "assistant";

    // Named content slots the assembler computes. A slot card emits the matching field; an empty/missing slot is skipped.
...
export type PromptItem =
  ...
  | { type: "plain"; text: string; role: PromptItemRole }
      // Chat-history marker. range slices the history: negative counts from the end, "end" is the full length, ALL (-1000) is the whole history.
  | { type: "chat"; rangeStart: number; rangeEnd: number | "end" };

    // One emitted prompt part: either a role message or the history placeholder (which the stream service expands into the conversation messages).
export type PromptPart =
  | { kind: "message"; role: PromptItemRole; text: string }
  | { kind: "chatHistory"; rangeStart: number; rangeEnd: number | "end" };

    // Content for each slot, pre-built by the assembler. Each is a finished string (already macro-expanded) plus the role it should be emitted as.
...
export const CHAT_RANGE_ALL = -1000;
    ...
    // Fixed assembly order as a template, used when a preset has no explicit promptTemplate so default behavior is unchanged. Tail ordering: chat history, then prefill, then the post-history end inject last. Users can reorder via a stored template.
export const DEFAULT_PROMPT_TEMPLATE: PromptItem[] = [
  ...
  { type: "slot", slot: "postHistory" },
    ...
    // Apply an innerFormat wrapper: replace {{slot}} with the content (RisuAI). No {{slot}} token means the format replaces entirely.
function applyInnerFormat(content: string, innerFormat?: string): string {
  ...
  return innerFormat.includes("{{slot}}")
    ...
    // Walk a template into ordered prompt parts. Empty slots and zero-length text blocks are dropped; the chat card becomes a chatHistory placeholder.
export function walkTemplate(
  ...
  return parts;
    ...
    // Parse a stored promptTemplate JSON string. Returns null (use default) on absent/invalid input or an unrecognized shape. Unknown card types are dropped.
export function parsePromptTemplate(
  ...
  try {
    ...
    // Resolve a chat card's [start, end) bounds against a history of length msgs. Mirrors RisuAI (negative = from end, ALL = whole).
