...
export type AssembleOpts = {
  ...
  history?: {
    ...
    time?: number;
      ...
      // Pre-mutated chat var store (start triggers); used over settings.vars so trigger writes reach the prompt + writeback.
...
export async function assembleForStream(
  ...
  fallbackSystemMessage: string | undefined,
      // Required: the caller owns the scoped context load. A fallback here would re-resolve convId without the userId, reopening the cross-user read this guards against.
  ...
  const scenario = primary?.scenario ?? "";
      ...
      // vars is mutated in place across expand() calls so writes reach later reads; the caller persists after the stream.
  ...
  if (exampleTurns.length > 0) {
    ...
    promptParts.splice(at, 0, ...exampleTurns);
      ...
      // system = only the leading run the stream hoists; later parts are emitted inline and must not be duplicated here.
  ...
  return {
    ...
    providerRouting: parseProviderRouting(preset?.providers),
        // Emitted as a prefill slot in promptParts; kept here for the GLM end-stub flag and the no-prefill-card fallback.
