...
export async function handleImageStream(apiKey: string, body: MediaStreamBody) {
  ...
  // AI Horde has no sync image endpoint; it polls through the video task flow.
  ...
  return streamResponse(async (writer) => {
    ...
    // Bytes must NOT go into the message text: a data: URI re-runs the markdown
    // pipeline over the whole base64 string per render, locking the page with
    // "Maximum call stack size exceeded". Text carries only a {{inlay::id}} token.
    const inlayMedia = (
      await Promise.all(
...
// Without a source image DashScope rejects with "Field required: input.media".
...
export async function handleVideoTaskStream(
  ...
  return streamResponse(async (writer) => {
    ...
    // The upstream render worker cannot fetch a data: URI, so i2v needs an http ref.
