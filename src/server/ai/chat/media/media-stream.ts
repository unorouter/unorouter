...
export async function handleImageStream(apiKey: string, body: MediaStreamBody) {
  ...
  return streamResponse(async (writer) => {
    ...
    // multi-megabyte PNG re-runs the markdown pipeline over the whole base64
    // string on every render, locking the page and overflowing the stack
    // ("Maximum call stack size exceeded") on an image the user is billed for
    // and never sees. The bytes ride finish-meta `inlayMedia`; the text carries
    // only a `{{inlay::id}}` token.
    const inlayMedia = (
      await Promise.all(
...
// Without a source image DashScope rejects the task with "Field required:
// input.media" / "img_url must be set".
...
export async function handleVideoTaskStream(
  ...
  return streamResponse(async (writer) => {
    ...
    // References pass through as-is, and the upstream render worker cannot
    // fetch a data: URI, so i2v needs an http reference.
