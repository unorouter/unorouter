    ...
    >
      ...
      {displayedUpdates.map((update, index) => {
        ...
        if (index === 0) {
          ...
          const duration = dayjs(updateDate).from(startedAt, true);

          if (
            dayjs(updateDate).diff(startedAt, "second") !== 0 &&
          ...
          const timeFromLast = dayjs(lastUpdateDate).from(updateDate, true);
...
export function StatusEventTimelineMaintenance({
  ...
  const labels = useStatusBlocksLabels();
  const duration = dayjs(maintenance.to).from(maintenance.from, true);
