...
export function ModelSelector(props: ModelSelectorProps) {
  ...
  const allModels: GroupedModel[] = rawModels.map((m) => ({
    name: m.name,
    vendorName: typeof m.vendor === "string" ? m.vendor : m.vendor?.name ?? "",
    tags: m.tags,
  }));
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        <div className="flex items-center gap-2 truncate">
          {selected && (
            <VendorIcon vendor={selected.vendorName} size={14} />
          )}
      ...
      <PopoverContent className="w-72 p-0" align="start">
        <Command>
          <CommandInput placeholder={t("CHAT.SEARCH_MODEL")} className="h-8 text-xs" />
          <CommandList>
