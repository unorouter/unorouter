...
function getIcon(vendor: string): IconComponent | null {
  ...
  if (!key) return null;
      ...
      // SSR on so the real icon is in the HTML (ssr:false flashed a spinner per card each refresh); client-side loading fallback is a neutral block, not a spinner.
