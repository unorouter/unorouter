...
export type ChatHelpersRef = {
  ...
  sendEmpty: () => Promise<void>;
    ...
    // In-memory runtime state, NEVER persisted (per-tab, per-stream). Object-backed like ChatState so it's one source with derived per-field atoms, but a plain atom (no storage) since none of it should survive reload. Helper refs / globalVars / localUserId stay separate below: each needs a storage tier this object can't give.
export type ChatRuntimeState = {
  // Active stream convId; generated by ensureConvId, mirrored from the thread.
  convId: string | null;
      // True once the history adapter's first load() resolved; the thread welcome waits on it so a conv URL shows a skeleton instead of flashing the empty welcome while messages load.
  historyLoaded: boolean;
      // Conversation settings/overrides drawer open; shared so the header badge and the actions menu drive the same drawer.
  settingsOpen: boolean;
      // Last stream failure; the history adapter persists the failed run as an error node instead of an empty ghost branch.
  ...
  speakingCharacterId: string | null;
      // Greeting picked on the empty-thread preview (0 = firstMessage, i = alternateGreetings[i-1]); thread-list initialize seeds branches from it.
...
export const chatHelpersAtom = atom<ChatHelpersRef | null>(null);
    ...
    // Per-user global macro vars (JSON map). localStorage-backed (can outgrow the cookie cap); read into chatContext, updated from finish-meta writeback.
export const globalVarsAtom = atomWithStorage<string>(
  ...
  { getOnInit: true },
    ...
    // Authoritative local-DB owner (real user id, or GUEST_USER_ID). Seeded server-side by UserIdProvider so it's correct on first client render with no auth-query race. Read via useLocalUserId() or chatStore.get(localUserIdAtom).
