...
import {
  ...
  readLocalPreset,
    ...
    // Streamed RP context from SQLocal. Characters travel as {binding, character} so the assembler honors per-character isActive/overrides.
export async function buildChatContextFromLocalDb(
  userId: number | undefined,
  convId: string,
      // First send on a new conv: the loadout says bindings are coming; wait so the character is in the prompt from turn 1.
  opts?: { expectBindings?: boolean },
): Promise<ChatContext | undefined> {
      // New conv: can run the same tick initialize() writes the row (OPFS lag). Retry briefly, else turn 1 ships with no loadout.
  ...
  const characters = characterRows
    ...
    .map((c) => ({ binding: c.binding, character: c.character! }));
      // Disabled entries never inject (Turso path filters enabled=true); drop them here for parity and to stay off the wire.
