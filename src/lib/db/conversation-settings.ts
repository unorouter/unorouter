import type { ConversationRow } from "@/lib/db/schema/rows";
    ...
    // Settings columns live on the conversations row; these helpers project them into the { convId, ...settings } shape callers expect.
...
export type ConversationSettingsProjection = ConversationRow & {
  convId: string;
    ...
    // The projection is just the row plus a convId alias: every settings column already lives on the row, so picking keys gains nothing.
