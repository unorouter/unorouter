...
// Collect every case-insensitive occurrence of each phrase, tagged with kind.
...
// Re-derive the matched evidence spans from a stored response. Deterministic over
// the SAME pattern arrays the signal matchers use, so the highlight always agrees
// with the verdict. Pure: no schema field needed. Returns the text split into an
// ordered, non-overlapping segment list (matched segments carry a kind).
export function highlightSpans(
  ...
  // model-name probe also flags cloud model names as foreign.
  if (probeLabel === "model-name")
    matches.push(
      ...collectPhrase(lower, cfg.cloudModelNamePatterns, "foreign"),
    );

  // CJK leak: each CJK char is its own match (the regex is global).
  ...
  // Resolve overlaps: earliest start wins, then longest; drop anything that
