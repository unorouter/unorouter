// RisuAI parseChatML port (src/ts/parser/chatML.ts): <|im_start|> blocks into
// role-tagged messages. Returns null when the input is not ChatML so callers
// fall back to a plain user prompt.
...
export function parseChatML(data: string): ChatMLMessage[] | null {
  ...
  return trimmed
    ...
    .map((v) => {
      ...
      // Risu strips <Thoughts> into a side channel; the V1 runLLM path only
      // consumes content.
