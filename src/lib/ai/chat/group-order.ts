    // Multi-character turn ordering (RisuAI groupOrder port): name-mention priority then weighted-random talkativeness, last speaker excluded.
...
export function groupOrder(
  ...
  const taken = new Set<string>();
      ...
      // Stage 1: name-mention priority in mention order. Risu compares each word against name chunks, so partial and multi-word names match.
  ...
  for (const w of words) {
    ...
    if (hit) {
      ...
      // Stage 2: weighted-random fill, seeded by member id + user text so a turn rolls the same speakers across regenerates. Risu default 0.5.
